...
const startIndex = content.indexOf("{participantsInThisRound.map(({player, pts, compPts");
const startOfDiv = content.indexOf("<div className=\\"grid grid-cols-3 gap-8\\">", startIndex - 100);
if(startOfDiv !== -1 && startOfDiv < startIndex) {
    // Found the wrapper
}
// Find the exact end of map
const endStr = `                                );
                            })}
                        </div>
                       </div>
                               ...
                               const theme = i === 0 ? {
                                ...
                                <div key={player.id} className={" " + theme.bg + " border " + theme.border + " rounded-[28px] p-[22px] relative transition-all flex flex-col " + theme.glow + " mx-auto"} style={{ width: '330px', height: '430px' }}>
                                   ...
                                   <div className="flex flex-col items-center justify-start flex-1 z-10 w-full mt-4">
                                      ...
                                      {/* Stats Box */}
                                      <div className="w-full h-[90px] bg-[#F8FAFC] rounded-[22px] border border-slate-100 flex items-stretch p-1 mb-4 shadow-sm z-10 mx-auto">
                                         {/* الأصوات */}
                                      ...
                                      {/* Badges Area */}
                                      <div className="w-full h-[90px] flex flex-wrap items-start justify-center gap-2 content-start overflow-hidden z-10 px-0">
                                         {hasDebt && (
                                            <div className="px-[12px] h-[36px] rounded-[18px] text-[11px] font-black flex items-center gap-1 border bg-rose-50 border-rose-100 text-rose-600">
                                               <Info size={12} />
                                               لم يسدد اشتراك
                                            </div>
                                         )}
                                         {didNotVote && (
                                            <div className="px-[12px] h-[36px] rounded-[18px] text-[11px] font-black flex items-center gap-1 border bg-orange-50 border-orange-100 text-orange-600">
                                               <MicOff size={12} />
                                               لم يصوّت للآخرين
                                            </div>
                                         )}
                                         {showTieBreakReasons && attendanceCount > 0 && (
                                            <div className="px-[12px] h-[36px] rounded-[18px] text-[11px] font-black flex items-center gap-1 border bg-emerald-50 border-emerald-100 text-emerald-600">
                                               <CheckCircle size={12} />
                                               حضور {attendanceCount} مرة
                                            </div>
                                         )}
                                         {showTieBreakReasons && lateCount > 0 && (
                                            <div className="px-[12px] h-[36px] rounded-[18px] text-[11px] font-black flex items-center gap-1 border bg-slate-100 border-slate-200 text-slate-600">
                                               <Clock size={12} />
                                               تأخر {lateCount} مرات
                                            </div>
                                         )}
                                         {showTieBreakReasons && earlyCount > 0 && (
                                            <div className="px-[12px] h-[36px] rounded-[18px] text-[11px] font-black flex items-center gap-1 border bg-sky-50 border-sky-100 text-sky-600">
                                               <FastForward size={12} />
                                               مبكر {earlyCount} مرة
                                            </div>
                                         )}
