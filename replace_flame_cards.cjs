const fs = require('fs');
let s = fs.readFileSync('src/flame.tsx', 'utf8');

// Replace getPlayerLevel
s = s.replace(
...
const newCard = `<div key={p.id} className="relative rounded-3xl p-3 sm:p-4 border shadow-sm flex flex-col items-center justify-center gap-2 overflow-visible group hover:-translate-y-1 transition-all" style={{ backgroundColor: lvl.bgColor || lvl.color || '#ffffff', borderColor: 'rgba(0,0,0,0.05)' }}>
                           ...
                           <div className="relative mt-2 mb-2 w-full flex justify-center">
                             ...
                             <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-full bg-white shadow-sm border-2 flex items-center justify-center relative z-10 mx-auto cursor-pointer hover:scale-105 active:scale-95 transition-transform" onClick={() => onPlayerClick(p)} style={{ borderColor: lvl.iconColor || lvl.color || '#e2e8f0' }}>
                           ...
                           <div className="text-center w-full mt-1">
                              ...
                              <div className="flex items-center justify-center gap-1 mt-1 mb-1 relative z-10 cursor-pointer w-max mx-auto px-4 rounded-xl hover:bg-black/5" onClick={() => onPlayerClick(p)}>
...
const newCandidate = `<div className="text-[11px] font-bold text-slate-500 mt-1">
                                    ...
                                    <div className="text-[11px] font-bold text-amber-600 mt-0.5 bg-amber-50 inline-block px-1.5 py-0.5 rounded">
                                       يفصله حضوره مبكر بـ {diff} مرات
                                    </div>`;

s = s.replace(oldCandidate, newCandidate);

fs.writeFileSync('src/flame.tsx', s);
console.log('done!');
